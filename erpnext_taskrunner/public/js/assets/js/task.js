...
export default function useTask(props, emit, isEditing, editedText, cancelTriggered) {
    ...
    const updateTimesheetDetail = async (projectName, taskName, status, timesheetDetail, description = '') => {
        ...
        try {
            const r = await callBackendHandler('toggle_timer', {project: projectName, docName: taskName, status: status, timesheetDetail: timesheetDetail, description: description}, null);
            return r.message;
        }
        catch (error) {
            emit('catch-error', error)
